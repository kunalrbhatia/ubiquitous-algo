...
export class StrategyManager implements IStrategyManager {
  ...
  async buildBasket(underlying: string, skipLiquidityCheck = false): Promise<StrategyLeg[] | null> {
    ...
    const futureExpiries = expiries.filter((exp) => {
      const expDate = dayjs(exp.replace(/([A-Z]{3})/, (m) => m.charAt(0) + m.slice(1).toLowerCase()), 'DDMMMYYYY').endOf('day');
      /* istanbul ignore next */
    ...
    const t0ExpDate = dayjs(expiryT0.replace(/([A-Z]{3})/, (m) => m.charAt(0) + m.slice(1).toLowerCase()), 'DDMMMYYYY').hour(15).minute(30);
    const t0DaysToExpiry = Math.max(0.01, t0ExpDate.diff(now, 'day', true));
    const t0 = t0DaysToExpiry / 365;

    const t1ExpDate = dayjs(expiryT1.replace(/([A-Z]{3})/, (m) => m.charAt(0) + m.slice(1).toLowerCase()), 'DDMMMYYYY').hour(15).minute(30);
